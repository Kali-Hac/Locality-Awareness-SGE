...
def main(_):
	...
	pre_task = Model
	frames_ps = dataset + '_CME/' + str(time_steps) + setting + '/'
	epochs = 400
	...
	if train_flag == '1' and Model != 'rev_rec_plus':
		...
		for i in ['x', 'y', 'z']:
			try:
				os.mkdir(folder_name + '_' + i + '_' + str(time_steps) + '_' + str(temperature) + '_' + Frozen + view + probe_type + 'pre_' + pre_task + LSTM_layer + change)
				print("### LSTM layers", LSTM_layer)
			except:
				pass
			train(folder_name + '_' + i + '_' + str(time_steps) + '_' + str(temperature)+ '_' + Frozen + view + probe_type + 'pre_' + pre_task + LSTM_layer + change, i, train_dataset=dataset)
		...
		for i in ['x', 'y', 'z']:
			try:
				os.mkdir(folder_name + '_' + i + '_' + str(time_steps) + '_' + str(temperature) + '_' + Frozen + view + probe_type + 'pre_' + pre_task)
			except:
				pass
			train(folder_name + '_' + i + '_' + str(time_steps) + '_' + str(temperature)+ '_' + Frozen + view + probe_type + 'pre_' + pre_task, i, train_dataset=dataset)
		# Prediction
		attention = 'BA'
		folder_name = dataset + '_' + attention
		folder_name = './Models/Gait_Encoding_models/' + folder_name
	...
	# direct evaluation
		...
		encoder_match_union_directly(X, y, t_X, t_y, './Models/CAGEs_RN_models',
		                                dataset + '_' + attention + '_RN_' + manner + '_' + str(
			                                ...
			                                Frozen) + view + probe_type + 'pre_' + Model + LSTM_layer + change, dataset)
